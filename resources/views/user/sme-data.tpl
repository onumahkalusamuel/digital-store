...
{block name=title}{$network|upper} SME Data{/block}
{block name=body}
    ...
    <div class="scrollable-container">
        <div class="scrollable-scroll-area">
            ...
            <form method="POST" action="{$route->urlFor('sme-data',['network' => $network])}"
                onsubmit="return ajaxPost('sme-data');" id="sme-data">
                ...
                <div class="card flex align-items-center">
                    <select name="bundle" class="input card-details" id="bundle">
                        ...
                        {foreach from=$priceList item=item}
                            <option value="{$item.units}">{$item.data} - #{$item.amount}</option>
                        {/foreach}
                    </select>
                </div>

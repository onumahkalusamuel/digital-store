...
final class VTUAirtime
{
    ...
    public function init(
        ...
        $this->view->display('user/vtu-airtime.tpl');
    ...
    public function confirm(
        ...
        // process request 
        if (empty($message)) {
            $vtuairtime = $this->$network->topUp($id, $data['phone'], $data['amount']);
        }

        // if not successful, drop error message 
        if (empty($message) && empty($vtuairtime['success'])) {
            if (empty($vtuairtime['code'])) $message = $vtuairtime['message'];
            else {
                $trackingId = uniqid();
                $this->logMessage('vtu-airtime-error', $trackingId . " :: " . json_encode($vtuairtime));
        ...
        if (empty($message)) {
            $this->trailLog->create([
                'data' => [
                    ...
                    'trans_ref' => $vtuairtime['trans_ref'],
                    'balance_before' => $balance_before,
                    'balance_after' => $balance_after,
                    'loyalty_points' => $loyalty_points,
                    'platform_id' => $vtuairtime['platform_id']
                ]
            ]);

            // notify the user of this transaction
            $this->sendMail->sendTransactionLog([
                'transaction' => "VTU Airtime - {$network}",
                'trans_ref' => $vtuairtime['trans_ref'],
                'amount' => $data['amount'],
                'destination' => $data['phone'],
                'balance_before' => $balance_before,
